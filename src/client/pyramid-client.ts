...
export class PyramidClient {
  ...
  getWorkflow(projectId: string): Promise<unknown> {
    return this.request<unknown>(
      'GET',
      `/v1/projects/${encodeURIComponent(projectId)}/workflow`,
    );
  }
  ...
  moveTask(taskId: string, body: unknown): Promise<unknown> {
    return this.request<unknown>(
      'PATCH',
      `/v1/tasks/${encodeURIComponent(taskId)}/move`,
      { body },
    );
  }
  ...
  setFieldValues(taskId: string, body: unknown): Promise<unknown> {
    return this.request<unknown>(
      'PATCH',
      `/v1/tasks/${encodeURIComponent(taskId)}/field-values`,
      { body },
    );
  }
  ...
  unarchiveTask(taskId: string): Promise<unknown> {
    return this.request<unknown>(
      'POST',
      `/v1/tasks/${encodeURIComponent(taskId)}/unarchive`,
    );
  }
  ...
  async deleteTask(taskId: string, hard: boolean): Promise<void> {
    await this.withPrecondition(taskId, (etag) =>
      this.request<unknown>(
        'DELETE',
        `/v1/tasks/${encodeURIComponent(taskId)}`,
        { query: { hard }, headers: { 'if-match': etag } },
      ),
    );
  }
  ...
  replyComment(commentId: string, body: unknown): Promise<unknown> {
    return this.request<unknown>(
      'POST',
      `/v1/comments/${encodeURIComponent(commentId)}/replies`,
      { body },
    );
  }
   ...
   */
  private async withPrecondition<T>(
    taskId: string,
    fn: (etag: string) => Promise<T>,
  ): Promise<T> {
