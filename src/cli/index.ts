...
function ensureDestructiveConfirm(opts: CliGlobalOptions): void {
  ...
  if (process.stdout.isTTY === true && process.stdin.isTTY === true) {
    ...
    renderUsageError(
      "this action is destructive — re-run with --yes to confirm.",
    );
  }
  renderUsageError(
...
function printHelp(cmds: Command[]): void {
  ...
  lines.push("");
  lines.push("Global flags: --json --project <P> --base-url <U> --yes/-y --no-color --quiet/-q --no-cache");
...
export async function runCli(argv: string[]): Promise<number> {
  ...
  if (cmd.op.name === "list_tasks" && parsed.raw.__mine) {
    ...
    if (mine) {
      ...
      const reparsed = parseCommandArgs(mine, args.filter((a) => a !== "--mine"));
