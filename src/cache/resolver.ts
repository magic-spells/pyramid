...
export class Resolver {
  ...
  async resolveProject(nameOrSlug: string): Promise<ProjectSummary> {
    ...
    const fuzzy = projects.filter(
      (p) => contains(p.slug, needle) || contains(p.name, needle),
    );
    ...
    throw new McpError(
      'project_not_found',
      `No project matches "${nameOrSlug}".`,
      { candidates: shortlist(nameOrSlug, projects.map((p) => p.slug)) },
    );
  }
  ...
  async resolveStatus(projectId: string, q: string): Promise<WorkflowStatus> {
    ...
    if (!match) {
      const stage = wf.stages.find(
        (s) => norm(s.key) === needle || norm(s.name) === needle,
      );
      if (stage) {
    ...
    if (!match) {
      const fuzzy = wf.statuses.filter(
        (s) => contains(s.key, needle) || contains(s.name, needle),
      );
    ...
    throw new McpError('status_not_found', `No status matches "${q}".`, {
      candidates: shortlist(q, wf.statuses.map((s) => s.name)),
    });
  }
  ...
  async resolveStage(projectId: string, q: string): Promise<WorkflowStage> {
    ...
    throw new McpError('stage_not_found', `No stage matches "${q}".`, {
      candidates: shortlist(q, wf.stages.map((s) => s.name)),
    });
  }
  ...
  async resolveUser(projectId: string, q: string): Promise<WorkflowMember> {
    ...
    throw new McpError('user_not_found', `No user matches "${q}".`, {
      candidates: shortlist(q, wf.members.map((m) => m.display_name)),
    });
  }
  ...
  async resolveLabel(
    ...
    throw new McpError('label_not_found', `No label matches "${q}".`, {
      candidates: shortlist(q, wf.labels.map((l) => l.name)),
    });
  }
...
function pick<T>(
  ...
  for (const tier of tiers) {
    ...
    if (tier.length > 1) {
      ...
      const code = isMcpAmbiguousCode(ambiguousCode)
        ? ambiguousCode
        : 'validation_failed';
      throw new McpError(
        code,
        `"${query}" is ambiguous — ${tier.length} matches.`,
...
function projectRole(v: unknown): ProjectRole {
  return (PROJECT_ROLES as readonly string[]).includes(str(v))
    ? (str(v) as ProjectRole)
    : 'member';
}
...
function shortlist(query: string, candidates: string[]): string[] {
  ...
  scored.sort(
    (a, b) => a.sub - b.sub || a.dist - b.dist || a.c.localeCompare(b.c),
  );
