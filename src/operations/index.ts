...
const whoami: Operation<Record<string, never>, WhoAmI> = {
  name: 'whoami',
  summary:
    'Report the authenticated user, their single workspace, and accessible projects.',
  input: z.object({}),
...
async function resolveTaskRef(ctx: OpContext, ref: string): Promise<TaskRef> {
  ...
  if (TASK_KEY_RE.test(trimmed)) {
    ...
    const hit = page.data
      .map(rec)
      .find((r) => (readStr(r.key) ?? '').toLowerCase() === needle);
    if (hit) {
      return {
        id: readStr(hit.id) ?? '',
  ...
  throw new McpError(
    'task_not_found',
    `"${ref}" is neither a task key (e.g. WEB-42) nor a UUID.`,
  );
}
  ...
  if (status !== undefined) {
    ...
    if (stage !== undefined) {
      ...
      if (resolved.stage_id !== resolvedStage.id) {
        throw new McpError(
          'status_not_in_stage',
          `Status "${status}" is not in stage "${stage}".`,
        );
      }
    }
...
async function buildCreateBody(
  ...
  const resolvedStatus = await resolveStatusWithStage(
    ctx,
    projectId,
    row.stage,
    row.status,
  ...
  if (row.custom_fields !== undefined) {
    body.field_values = await resolveFieldValues(
      ctx,
      projectId,
      row.custom_fields,
    );
...
async function buildStageResponsibilities(
  ...
  const byStage = new Map<
    string,
    { stage_id: string; owner_id?: string; reporter_id?: string }
  >();
...
async function resolveFieldValues(
  ...
  for (const cf of fields) {
    ...
    if (def === undefined) {
      throw new McpError(
        'field_not_found',
        `No custom field matches "${cf.field}".`,
      );
    }
...
const taskCreate: Operation<TaskCreateInput, TaskDetail> = {
  name: 'create_task',
  summary:
    'Create a task in a project. Accepts names/keys for stage/status/owner/reporter/labels.',
...
async function resolveTemplateId(
  ...
  const match = wf.templates.find(
    (t) => t.id === template || t.name.toLowerCase() === needle,
  );
  if (match) return match.id;
  throw new McpError(
    'validation_failed',
    `No task template matches "${template}".`,
...
const taskUpdate: Operation<TaskUpdateInput, TaskDetail> = {
  ...
  async run(input, ctx): Promise<TaskDetail> {
    ...
    if (input.owner !== undefined || input.reporter !== undefined) {
      ...
      if (input.owner !== undefined) {
        entry.owner_id =
          input.owner === null
            ? null
            : (await ctx.resolver.resolveUser(projectId, input.owner)).id;
      }
    ...
    for (const labelId of addLabelIds ?? []) {
      await failingSubUpdate('add_label', () =>
        ctx.client.addTaskLabel(ref.id, labelId),
      );
    }
    ...
    for (const labelId of removeLabelIds ?? []) {
      await failingSubUpdate('remove_label', () =>
        ctx.client.removeTaskLabel(ref.id, labelId),
      );
    }

    // custom_fields -> PATCH /tasks/{id}/field-values { field_values: {...} }.
    if (input.custom_fields !== undefined) {
      const fieldValues = await resolveFieldValues(
        ctx,
        projectId,
        input.custom_fields,
      );
 ...
 */
async function failingSubUpdate<T>(
  which: string,
  fn: () => Promise<T>,
): Promise<T> {
  try {
...
const taskMove: Operation<TaskMoveInput, TaskDetail> = {
  ...
  async run(input, ctx): Promise<TaskDetail> {
    ...
    if (input.after_task !== undefined && input.before_task !== undefined) {
      throw new McpError(
        'validation_failed',
        'Pass at most one of after_task / before_task.',
      );
    }
...
const taskArchive: Operation<TaskArchiveInput, TaskDetail> = {
  name: 'archive_task',
  summary:
    'Soft-archive (or unarchive) a task. Reversible — not destructive-gated.',
...
const taskDelete: Operation<
  TaskDeleteInput,
  { id: string; key: string; deleted: true }
> = {
  name: 'delete_task',
  summary:
...
const commentAdd: Operation<CommentAddInput, TaskComment> = {
  name: 'add_comment',
  summary:
    "Add a stage-scoped comment to a task (defaults to the task's current stage).",
  ...
  async run(input, ctx): Promise<TaskComment> {
    ...
    const wf = projectId
      ? await ctx.resolver.getWorkflow(projectId)
      : EMPTY_WORKFLOW;
...
const commentReply: Operation<CommentReplyInput, TaskComment> = {
  ...
  summary:
    'Reply to a ROOT comment (one level only; replying to a reply -> reply_depth_exceeded).',
  ...
  async run(input, ctx): Promise<TaskComment> {
    ...
    if (
      parentId !== undefined ||
      (threadRootId !== undefined && threadRootId !== selfId)
    ) {
      throw new McpError(
    ...
    const projectId = taskId
      ? (await resolveTaskRef(ctx, taskId)).projectId
      : '';
    const mentionIds = await resolveMentionIds(ctx, projectId, input.mentions);
...
const searchTasks: Operation<SearchTasksInput, Page<TaskSummary>> = {
  name: 'search_tasks',
  summary:
    'Full-text search tasks across the workspace by title/key/content. Returns one page.',
...
const listComments: Operation<ListCommentsInput, Page<TaskComment>> = {
  ...
  async run(input, ctx): Promise<Page<TaskComment>> {
    ...
    const wf = ref.projectId
      ? await ctx.resolver.getWorkflow(ref.projectId)
      : EMPTY_WORKFLOW;
...
export const operationsByName: Map<string, Operation> = new Map(
